  ...
  return (
    <View style={styles.container}>
    	<View style={styles.profileView}>
	    	...
	    	<Image source={user?.avatar ? { uri: user.avatar } : DefaultAvatar} style={styles.image} />
    	...
    	<View style={styles.content}>
    		<View style={styles.dataView}>
    			...
    			<Text style={styles.data}>Modelo - {user?.cab?.model || "Exemplo de modelo"}</Text>
    		</View>
    		<View style={styles.dataView}>
    			<View style={styles.iconView}>
	    			<Ionicons name="color-palette" size={20} color="#47ca6c" />
    			</View>
    			<Text style={styles.data}>Cor - {user?.cab?.color || "Exemplo de cor"}</Text>
    		</View>
    		<View style={styles.dataView}>
    			<View style={styles.iconView}>
	    			<Foundation name="credit-card" size={20} color="#47ca6c" />
    			</View>
    			<Text style={styles.data}>Placa - {user?.cab?.plateNumber || "Exemplo de placa"}</Text>
    		</View>
    	</View>
    	
    	<TouchableOpacity onPress={() => driverStorageService.clearDriver()} style={{ backgroundColor: "black", alignSelf: 'stretch', borderRadius: 20, justifyContent: 'center', alignItems: 'center', padding: 10 }}>
