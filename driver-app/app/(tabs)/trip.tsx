...
export default function TripScreen() {
  ...
  useEffect(() => {
    const fetchTrips = async () => {
      ...
      setTripsHistory(
        trips.filter(dto => dto.status === TripResponseDTOStatus.COMPLETED || dto.status === TripResponseDTOStatus.CANCELLED).map((tripDTO) => driverStorageService.generateTrip(tripDTO))
      );
    };
    fetchTrips();
  }, [tab]);
  ...
  useEffect(() => {
    const getRequests = async () => {
      if (user) {
        const tripsRequests = await driverStorageService.getTripsRequests();
        setTripRequests(tripsRequests.map((tripDTO) => driverStorageService.generateTrip(tripDTO)));
      }
    };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.tabs}>
        ...
        >
          <Text
            style={[styles.tabText, tab === "atuais" && styles.activeTabText]}
          >
            Corridas Atuais
          </Text>
        </TouchableOpacity>
        ...
        >
          <Text
            style={[
              styles.tabText,
              tab === "anteriores" && styles.activeTabText,
            ]}
          >
            Corridas Anteriores
          </Text>
      ...
      {tab === "atuais" ? (
        tripRequests.length ? (
            ...
            renderItem={({ item: currentTrip }) => (
              <View style={styles.card}>
                <View style={styles.userInfo}>
                  <Image
                    source={currentTrip.customer.avatar ? { uri: currentTrip.customer.avatar } : DefaultAvatar}
                    style={styles.avatar}
                  />
                  <View>
                    <Text style={styles.name}>
                      {currentTrip.customer.name || 3}
                    </Text>
          ...
          <Text style={{ textAlign: "center", marginTop: 24 }}>
            Nenhuma solicitação de corrida no momento.
          </Text>
        )
      ) : (
        <>
            ...
            {finishedTrip && (
              <>
                <Text style={styles.modalText}>
                  Obrigado por realizar a corrida com{" "}
                  {finishedTrip.customer.name}.
                </Text>
                <Text style={styles.modalText}>
                  De: {finishedTrip.origin.name}
                </Text>
                <Text style={styles.modalText}>
                  Para: {finishedTrip.destination.name}
                </Text>
                <Text style={styles.modalText}>
                  Valor: R${" "}
                  {finishedTrip.payment.amount.toString().replace(".", ",")}
                </Text>
                <Text style={styles.modalText}>
                  Avaliação: {"⭐".repeat(finishedTrip.rating)} (
                  {finishedTrip.rating})
                </Text>
              </>
            )}
      ...
      <Modal visible={cancelModalVisible} transparent animationType="fade">
        <View style={styles.modalBackground}>
          <View style={styles.modalContainer}>
            ...
            <Text
              style={[styles.modalText, { fontSize: 48, marginVertical: 16 }]}
            >
              😢
            </Text>
            <Text style={styles.modalText}>Você cancelou a viagem.</Text>
            <TouchableOpacity
